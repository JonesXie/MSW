  ...
  <div class="Orderdetail">
    ...
    <div class="order_type">
        <div class="order_type_header">
            <ul>
                <li>{{shopName}} ><span style="float:right" v-if="deliveryModeType==0">卖家自取</span> <span style="float:right" v-else> 物流配送</span></li>
    ...
    <div class="order_img" v-for="(item,index) in productList" :key="index">
     ...
     <div class="order_las">
         <ul>
               <li>配送方式 <span class="fr" v-if="deliveryModeType==0">卖家自取 <span style="color:#ff0000"> ¥ 0.00</span></span>
...
<script>
  ...
  export default {
    ...
    //       let mouthData = this.postData.split('/')[1];
    //       let yearData = this.postData.split('/')[0];
    //       let moutharr = ['01', '03', '05', '07', '08', '10', '12'];
    //       // console.log(moutharr.includes(mouthData))
    //       if (moutharr.includes(mouthData)) {
    //         passDate = passDate + 1;
    //         if (passDate == 31) {
    //           passDate = 0;
    //           mouthData = mouthData + 1;
    //           yearData = yearData;
    //           if (mouthData == 12) {
    //             mouthData = 0;
    //             yearData = yearData + 1;
    ...
    //         }
    //       }
    //       this.postData = `${yearData}/${mouthData}/${passDate}`;
        });
      }
    },
    mounted () {
