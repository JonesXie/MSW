      ...
      <div v-for="(item,index) in goodsList" :key="index">
        <Group class="input_wrap">
          ...
          <cell title="长度：" class="cell_input">
            <div>
              <input style="text-align: right;" type="number" :placeholder="`长度在${sizeArr[1]}mm~${sizeArr[3]}mm之间`" v-model="item.length" @on-blur="isArea(index,'length')" v-number-only><em>mm</em>
            </div>
          </cell>
          <cell  title="宽度：">
            <div>
              <input style="text-align: right" type="number" v-model="item.width" @on-blur="isArea(index,'width')"
